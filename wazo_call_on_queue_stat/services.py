...
class StatCallOnQueueService():
    ...
    def get_stat_for_agent(self, agent_id, tenant_uuid, **args):
        result = self.dao.get_call_on_queue_stat_by_agent(agent_id=agent_id, tenant_uuid=tenant_uuid, **args)
        return CallOnStatQueueResultSchema().dump(result)
    
    def get_stat_for_queue(self, queue_id, tenant_uuid, **args):
        result = self.dao.get_call_on_queue_stat_by_queue(queue_id=queue_id, tenant_uuid=tenant_uuid, **args)
        return CallOnStatQueueResultSchema().dump(result)
